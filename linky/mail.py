...
class send_email(object):
    ...
    def __send(self):
        r = requests.post(self.__mailgunURL, auth=HTTPBasicAuth('api', self.__apikey), data=self.mailgun_send_dict)
        self.mail_send_status = r.status_code

    def mail_link(self):
        logger.info('Sending email to %s, Subject: %s' % (self.email, self.__subject))
        self.__send()
